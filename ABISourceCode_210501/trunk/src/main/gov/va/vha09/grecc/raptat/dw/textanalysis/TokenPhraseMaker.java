...
public class TokenPhraseMaker {
  ...
  public interface ITokenPhraseReplacementValidation {
    boolean validate(RaptatTokenPhrase similar, RaptatTokenPhrase foundTokenPhrase);
  }
  ...
  private class RaptatTokenPhrases {
    ...
    public boolean doesLhsPhraseCoverRhsPhrase(final RaptatTokenPhrase lhs,
      ...
      boolean rv = false;
      final int rhs_end_offset_as_int = rhs.get_end_offset_as_int();
      final int lhs_end_offset_as_int = lhs.get_end_offset_as_int();
      if (rhs_end_offset_as_int > lhs_end_offset_as_int) {
        final int rhs_start_offset_as_int = rhs.get_start_offset_as_int();
        final int lhs_start_offset_as_int = lhs.get_start_offset_as_int();
    ...
    public RaptatTokenPhrase doesPhraseCoverExisting(final RaptatTokenPhrase foundTokenPhrase) {
      for (final RaptatTokenPhrase existingPhrase : getPhrases()) {
        if (doesLhsPhraseCoverRhsPhrase(foundTokenPhrase, existingPhrase)) {
    ...
    /**
     * TODO -- review this; it makes for cleaner lists, but it may be preferable to have the
     * unassigned entries.
     */
    ...
    /**
     * Iterate through phrases that overlap with the same labels, removing all the smaller ones.
     */
    public void trimLabelsFromOverlappedPhrases() {
      ...
      final Comparator<RaptatTokenPhrase> overlapIsEqual = new Comparator<RaptatTokenPhrase>() {
        ...
        /**
         * Comparison to find out of two phrases overlap and thus are equal, or if not, push the
         * other phrases to end.
         */
        @Override
        public int compare(RaptatTokenPhrase largerPhrase, RaptatTokenPhrase smallerPhrase) {
          ...
          final int smallerPhrase_start = smallerPhrase.get_start_offset_as_int();
          final int smallerPhrase_end = smallerPhrase.get_end_offset_as_int();
          final int largerPhrase_start = largerPhrase.get_start_offset_as_int();
          /**
           * substract length of smaller from larger in order to find smaller start between larger
           * endpoints
           */
          final int largerPhrase_end =
              largerPhrase.get_end_offset_as_int() - (smallerPhrase_end - smallerPhrase_start);
      ...
      /**
       * Get list of token phrases sorted with all elements that overlap near the top and all other
       * elements near the bottom.
       */
      final List<RaptatTokenPhrase> sorted =
          getPhrases().stream().sorted((l, r) -> r.getPhraseLength().compareTo(l.getPhraseLength()))
              .collect(Collectors.toList());

      /**
       * Get all the labels in this set, such overlaps can be reviewed on a per-label basis
       */
      ...
      for (final Label labelToReview : allLabels) {
        ...
        final Predicate<RaptatTokenPhrase> containsLabel =
            x -> x.getPhraseLabels().contains(labelToReview);

        /**
         * Filter by label
         */
        final List<RaptatTokenPhrase> phrasesFilteredByLabel =
            sorted.stream().filter(containsLabel).collect(Collectors.toList());

        for (int index = 0; index < phrasesFilteredByLabel.size(); index++) {
          final RaptatTokenPhrase phraseToCheck = phrasesFilteredByLabel.get(index);

          /**
           * Check all the other phrases in this list, which will always be smaller than that
           * checked.
           */
          for (final RaptatTokenPhrase phrasesInSet : phrasesFilteredByLabel) {
            ...
            if (phrasesInSet.equals(phraseToCheck) == false) {
               ...
               */
              final boolean overlaps = overlapIsEqual.compare(phrasesInSet, phraseToCheck) == 0;
              if (overlaps && phrasesInSet.getPhraseLength() < phraseToCheck.getPhraseLength()) {
    ...
    /**
     * Iterate through phrases that overlap with the same labels, removing all the smaller ones.
     * Unlike trimLabelsFromOverlappedPhrasesUnfiltered(), this method ignores the labels. That is,
     * only the long phrase remains if overlapped.
     */
    public void trimLabelsFromOverlappedPhrasesUnfilteredGG() {
      ...
      final Comparator<RaptatTokenPhrase> overlapIsEqual = new Comparator<RaptatTokenPhrase>() {
        ...
        public int compare(final RaptatTokenPhrase firstPhrase,
          ...
          if (secondPhrase.get_end_offset_as_int() > firstPhrase.get_start_offset_as_int()
              && secondPhrase.get_start_offset_as_int() < firstPhrase.get_end_offset_as_int()) {
            return 0;
          }
          return -1;
        }
      ...
      /*
       * Remove any shorter length phrases. Considers phrases sequentially from first to last. Any
       * phrases with higher indices in the list that overlap are removed.
       *
       * First sort phrases inversely according to length, then loop through the list.
       */
      final List<RaptatTokenPhrase> sorted =
          getPhrases().stream().sorted((l, r) -> r.getPhraseLength().compareTo(l.getPhraseLength()))
              .collect(Collectors.toList());
      final ListIterator<RaptatTokenPhrase> testIterator = sorted.listIterator();
      while (testIterator.hasNext()) {
        final RaptatTokenPhrase initialPhrase = testIterator.next();
        while (testIterator.hasNext()) {
          final RaptatTokenPhrase testPhrase = testIterator.next();
          if (overlapIsEqual.compare(initialPhrase, testPhrase) == 0) {
            testIterator.remove();
          }
        }
      }
  ...
  /**
   * @param textDocumentLines
   * @param index_range_fsm_defn
   * @param indexValueTokenPhrasesForDocument
   * @param site_fsm_defn
   * @return
   */
  public List<RaptatTokenPhrase> getTokenPhrases(
      final LinkedList<TextDocumentLine> textDocumentLines,
      final FSMFeatureLabeler fsm_feature_labeler) {

    final List<RaptatTokenPhrase> resultPhraseList =
        new ArrayList<>(ESTIMATED_MAX_TOKEN_PHRASES_IN_DOC);
    ...
    /*
     * 7/29/19 - DAX - Changes to document lines
     */
    for (final TextDocumentLine documentLine : textDocumentLines) {
      final List<RaptatTokenPhrase> tokenPhrases =
          getLineTokenPhrasesGG(documentLine, fsm_feature_labeler);
  ...
  public List<RaptatTokenPhrase> getTokenPhrases(final List<TextDocumentLine> textDocumentLines,
    ...
    GeneralHelper.errorWriter("getTokenPhrases() not implemented in TokenPhraseMaker Class");
    System.exit(-1);
    return null;
  }

  private void add_index_value_label(final RaptatTokenPhrase foundTokenPhrase) {

    foundTokenPhrase.addPhraseLabel(RaptatTokenPhrase.Label.createIndexValueLabel());
  ...
  private void add_site_label(final RaptatTokenPhrase foundTokenPhrase) {
    foundTokenPhrase.addPhraseLabel(new RaptatTokenPhrase.Label("Site"));

  }

  private void add_unassigned_label(final RaptatTokenPhrase foundTokenPhrase) {
    foundTokenPhrase.addPhraseLabel(RaptatTokenPhrase.Label.createUnassignedLabel());
  ...
  private List<RaptatTokenPhrase.Label> findLabelsForTokenPhrase(
      final RaptatTokenPhrase found_token_phrase, final IRaptatTokenText string_eval,
      final FSMFeatureLabeler fsm_feature_labeler, final MatchPatternValidate validationCallback) {
    // /*
    ...
    if (text.endsWith(".")) {
      final RaptatToken raptatToken =
          found_token_phrase.getTokensAsList().get(found_token_phrase.getTokensAsList().size() - 1);
      String tokenStringPreprocessed = raptatToken.getTokenStringPreprocessed();
      tokenStringPreprocessed =
      ...
      raptatToken.setEndOff(String.valueOf(Integer.parseInt(raptatToken.getEndOffset()) - 1));
    ...
    if (fsm_feature_labeler != null) {
      try {
        fsm_feature_labeler.identify(potential_phrase, MatchType.Exact, validationCallback);
    ...
    final Map<String, FSMFeatureData> map = potential_phrase.get_fsm_feature_map();
    for (final String key : map.keySet()) {
      ...
      if (label.get_feature_value() != MatchPattern.None.get_match_pattern_name()) {
         ...
         * e.g., IndexType, ABI
         */
        final RaptatTokenPhrase.Label found_label =
   ...
   * @return
   */
  private List<RaptatTokenPhrase> getLineTokenPhrasesGG(final TextDocumentLine line,
      final FSMFeatureLabeler fsm_feature_labeler) {

    /*
     * x -> x.getTokenStringPreprocessed() is a lambda method defined in IRaptatTokenStringEval that
     * permits specification of which string method to use on the RapTAT token. Included here to
     * demonstrate how to generalize approach
     */
    ...
    /**
     * Create a validation against the entire line. The goal here is to permit a check for back
     * lookups, comparing what was found as an item against what exists in the line. If something is
     * found in the phrase search that does not exist in the line itself, due to negative lookup,
     * then it's not valid.
     */
    ...
    for (int startIndex = 0; startIndex < tokens.size(); startIndex++) {
      ...
      if (startIndex + RaptatConstants.MAX_TOKENS_DEFAULT < endIndex) {
        endIndex = startIndex + RaptatConstants.MAX_TOKENS_DEFAULT;
      }

      while (endIndex > startIndex) {
        final List<RaptatToken> candidates = tokens.subList(startIndex, endIndex);
        final RaptatTokenPhrase foundTokenPhrase = new RaptatTokenPhrase(candidates);

        final MatchPatternValidate validationCallback = new MatchPatternValidate() {

          /**
           * Use the matching details of the token phrase to review the original sentence, looking
           * to see if the found phrase exists in the same place as in the sentence. This is
           ...
           */
          @Override
          public boolean validate(final MatchDetails<RaptatToken> foundMatchDetails) {
            final String tokenStringPreprocessed = original_line.getTokenStringPreprocessed();

            final Pattern pattern = foundMatchDetails.getMatcher().pattern();
            final Matcher matcher =
                pattern.matcher(tokenStringPreprocessed.replaceAll("^\\s*", ""));

            boolean validated = false;

            while (matcher.find()) {
              final int foundEndOffset = foundMatchDetails.get_end();
              final int matcher_end = matcher.end() - matcher.start();
        ...
        final List<RaptatTokenPhrase.Label> labels = findLabelsForTokenPhrase(foundTokenPhrase,
            getTokenText, fsm_feature_labeler, validationCallback);

        /**
         * Were labels found, and if so add the phrase, otherwise check for indexValue
         */
        if (labels != null && labels.isEmpty() == false) {

          /**
           * BEGIN - 8/12/19 - DAX - Added mechanism to compare new "found" token with what's
           * already in the collection. If the new one covers the old one, replace
           *
      ...
      if (endIndex == startIndex) {
        final List<RaptatToken> token_sublist = tokens.subList(startIndex, startIndex + 1);
        final RaptatTokenPhrase foundTokenPhrase = new RaptatTokenPhrase(token_sublist);
   ...
   * @return
   */
  private List<RaptatTokenPhrase> getTokenPhrasesFromAnnotatedPhrase(final TextDocumentLine line,
      final FSMFeatureLabeler fsm_feature_labeler) {
    ...
    /*
     * 7/29/19 - DAX - get list of tokens from TextDocumentLine Trying at first a simple split,
     * since the line does not have the immediate benefit of the tokenizing from the sentence
     * detection.
     */
    ...
    /**
     * Create a validation against the entire line. The goal here is to permit a check for back
     * lookups, comparing what was found as an item against what exists in the line. If somehting is
     * found in the phrase search that does not exist in the line itself, due to negative lookup,
     * then it's not valid.
     */
    final RaptatTokenFSM original_line = new RaptatTokenFSM(line.lineText);
  ...
  private void identifyAllIndexValuesInALine(final TextDocumentLine line,
      final FSMDefinition index_value_fsm_defn, final FSMDefinition index_range_fsm_defn,
    ...
    if (index_value_fsm_defn != null) {
      final List<MatchPattern> index_elements = index_value_fsm_defn.get_match_patterns("INDEX");
      final List<MatchPattern> range_elements =
      ...
      /**
       * In order to loop through and capture all the IndexValues from a sentence, keep looping,
       * resetting the token by substring at each loop, until start offset == end offset
       */
      while (fsm_token != null) {
        final IRaptatTokenText token_text_callback = x -> x.getTokenStringPreprocessed();
        ...
        for (final MatchPattern element : index_elements) {
          ...
          if (match_details != null) {
            ...
            /**
             * Place the match into the "lookup", such that later it can be used to evaluate which
             * RaptatTokenPhrase is associated by offset
             */
            MatchDetails<RaptatTokenFSM> cloned_match =
                match_details.clone_to_offset(match_details, startOffset);

            int substring_delta = match_details.get_end();
            /**
             * Check for patterns matching "range" elements (e.g., 1.0 to 2.0) in order to exclude
             * them.
             */
            for (final MatchPattern range_element : range_elements) {
              final MatchDetails<RaptatTokenFSM> range_match_details =
              ...
              if (range_match_details != null) {
                ...
                match_details = null;
                cloned_match =
                    range_match_details.clone_to_offset(range_match_details, startOffset);
                substring_delta = range_match_details.get_end();
                break;
              }
            }

            stringToProccess = stringToProccess.substring(substring_delta);

            if (match_details != null) {
              index_value_start_offset_to_match.put(cloned_match.get_start(), cloned_match);
              /*
               * re-calculate to get the next piece of the "sentence"
               */
              startOffset = cloned_match.get_end();

            }

            fsm_token = stringToProccess.length() > 0 ? new RaptatTokenFSM(stringToProccess) : null;
  ...
  private static List<RaptatToken> generateRaptatTokensForLine(final TextDocumentLine line) {
    ...
    for (int index = 0; index < pieces.length; index++) {
      ...
      if (piece.trim().length() > 0) {
        final RaptatToken raptatToken = RaptatTokenHelper.AsPreprocessed(piece, start, end, index);
        if (raptatToken.getTokenStringPreprocessed().isEmpty() == false) {
