...
public class TextAnalyzer {
  ...
  private static final Collection<String> stopWordSet =
      Collections.unmodifiableCollection(new HashSet<>(Arrays.asList(RaptatConstants.STOP_WORDS)));
  ...
  static {
    ...
    for (final String curString : curArray) {
      TextAnalyzer.openNLPToLVGMap.put(curString, RaptatConstants.LVG_NOUN_BITFLAG);
    }
    curArray = RaptatConstants.OPEN_NLP_PRONOUNS;
    for (final String curString : curArray) {
      TextAnalyzer.openNLPToLVGMap.put(curString, RaptatConstants.LVG_PRONOUN_BITFLAG);
    }
    curArray = RaptatConstants.OPEN_NLP_ADJECTIVES;
    for (final String curString : curArray) {
      TextAnalyzer.openNLPToLVGMap.put(curString, RaptatConstants.LVG_ADJECTIVE_BITFLAG);
    }
    curArray = RaptatConstants.OPEN_NLP_MODALS;
    for (final String curString : curArray) {
      TextAnalyzer.openNLPToLVGMap.put(curString, RaptatConstants.LVG_MODAL_BITFLAG);
    }
    curArray = RaptatConstants.OPEN_NLP_ADVERBS;
    for (final String curString : curArray) {
      TextAnalyzer.openNLPToLVGMap.put(curString, RaptatConstants.LVG_ADVERB_BITFLAG);
    }
    curArray = RaptatConstants.OPEN_NLP_VERBS;
    for (final String curString : curArray) {
      TextAnalyzer.openNLPToLVGMap.put(curString, RaptatConstants.LVG_VERB_BITFLAG);
    }
  }
  ...
  public TextAnalyzer(final String dictionaryPath, final HashSet<String> acceptedConcepts) {
    this();
  ...
  public void addSectionMatcher(final String patternName, final String patternToMatch) {
    this.textSplitter.addSectionMatcher(patternName, patternToMatch);
  }

  public void addSentenceAndSectionMatchers(final List<RaptatPair<String, String>> sentenceMatchers,
    ...
    if (matchers != null) {
      for (final SentenceSectionTaggerType type : matchers.keySet()) {
        if (type.equals(SentenceSectionTaggerType.SENTENCE)) {
          for (final RaptatPair<String, String> matcherPair : matchers.get(type)) {
            addSentenceMatcher(matcherPair.left, matcherPair.right);
          }
        }
  ...
  public void addSentenceMatcher(final String patternName, final String patternToMatch) {
  ...
  public String[] getNegatedSentenceCue(final String sentenceText) {
    ...
    for (int i = 0; i < tokenStrings.length; i++) {
      final RaptatPair<List<String>, List<Integer>> processedList =
    ...
    final String[] negationContext = this.negationContextAnalyzer.getContextCue(tokenStrings);
  ...
  public String[] getNegatedSentenceTokensTest(final String sentenceText) {
    ...
    final String[] negationContext =
        this.negationContextAnalyzer.getContext(tokenStrings, RaptatConstants.NEGATION_TAG);

    return negationContext;
  }

  public String[] getNegatedSentenceTokensTest(final String[] sentenceTokens) {
    final String[] negationContext =
        this.negationContextAnalyzer.getContext(sentenceTokens, RaptatConstants.NEGATION_TAG);
  ...
  public RaptatPair<List<RaptatToken>, List<RaptatToken>> getSentenceTokens(
      final String sentenceText, final Span sentenceSpan, int tokenIndex) {
    ...
    final String[] preProcessedTokenStrings = this.stemmer.preProcess(baseTokenStrings);
    final String[] reasonNoMedsContext = this.reasonNoMedsContextAnalyzer
        .getContext(preProcessedTokenStrings, RaptatConstants.REASON_NO_MEDS_TAG);
    ...
    final RaptatPair<String[], Mark[]> negationContextAndMarks = this.negationContextAnalyzer
        .getContextAndContextMarks(preProcessedTokenStrings, RaptatConstants.NEGATION_TAG);
    ...
    final RaptatPair<String[], Mark[]> uncertainContextAndMarks = this.uncertainContextAnalyzer
        .getContextAndContextMarks(preProcessedTokenStrings, RaptatConstants.UNCERTAIN_TAG);
    ...
    final List<RaptatToken> rawTokenList = new ArrayList<>(baseTokenStrings.length);
    final List<RaptatToken> processedTokenList = new ArrayList<>(baseTokenStrings.length);
    ...
    for (int i = 0; i < preProcessedTokenStrings.length; i++) {
      ...
      newToken = new RaptatToken(baseTokenStrings[i], preProcessedTokenStrings[i], stemString,
          ...
          negationMarks[i].toString(), negationContext[i].equals(RaptatConstants.NEGATION_TAG),
          uncertainContext[i].equals(RaptatConstants.UNCERTAIN_TAG), tokenIndex++);
  ...
  public String[] getUncertainSentenceCue(final String sentenceText) {
    ...
    tokenStrings = processedTokenStrings.toArray(tokenStrings);
    final String[] uncertainContext = this.uncertainContextAnalyzer.getContextCue(tokenStrings);
  ...
  public String[] getUncertainSentenceTokensTest(final String sentenceText) {
    final String[] tokenStrings = this.tokenizer.tokenize(sentenceText);
    final String[] uncertainContext =
        this.uncertainContextAnalyzer.getContext(tokenStrings, RaptatConstants.UNCERTAIN_TAG);

    return uncertainContext;
  }

  public String[] getUncertainSentenceTokensTest(final String[] sentenceTokens) {
    final String[] uncertainContext =
        this.uncertainContextAnalyzer.getContext(sentenceTokens, RaptatConstants.UNCERTAIN_TAG);
  ...
  public RaptatDocument processDocument(final String pathToTextDocument) {
    ...
    if (inputText != null) {
      ...
      final RaptatDocument resultDocument = processText(inputText, pathToTextDocument);
  ...
  public RaptatDocument processText(final String inputText, final String documentPath) {
    ...
    final Map<Span, Set<String>> sentenceSpansAndContext =
        this.textSplitter.detectSentenceSpans(inputText);

    final List<AnnotatedPhrase> theSentences = new ArrayList<>(sentenceSpansAndContext.size());
    final RaptatDocument theDocument = new RaptatDocument();
    theDocument.setTextSourcePath(documentPath);

    final List<RaptatToken> processedTokensAsList =
        new ArrayList<>(RaptatConstants.TOKENS_PER_DOCUMENT);
    final List<RaptatToken> rawTokensAsList = new ArrayList<>(RaptatConstants.TOKENS_PER_DOCUMENT);

    if (!sentenceSpansAndContext.isEmpty()) {

      AnnotatedPhrase previousSentence = null;

      final List<Span> sentenceSpans = new ArrayList<>(sentenceSpansAndContext.keySet());
      ...
      for (final Span curSpan : sentenceSpans) {
        ...
        sentenceTokens = getSentenceTokens(sentenceText, curSpan, tokenIndex);
        tokenIndex += sentenceTokens.left.size();

        final Set<String> sentenceConcepts = sentenceSpansAndContext.get(curSpan);
        final String annotatorID = "RaptatSentence_" + UniqueIDGenerator.INSTANCE.getUnique();
        final String annotatorName = "RaptatSentence";
        final AnnotatedPhrase newSentence =
        ...
        if (this.facilitatorBlockerDictionary != null) {
          final List<AnnotatedPhrase> dictionaryAnnotationsForSentence =
      ...
      processedTokensAsTree = RaptatToken.generateIndexedTree(processedTokensAsList);
      rawTokensAsTree = RaptatToken.generateIndexedTree(rawTokensAsList);
    }

    List<TextDocumentLine> documentLines =
        getTextDocumentLines(inputText, processedTokensAsTree, rawTokensAsTree, theDocument);
    ...
    if (this.generateTokenPhrases) {
      /**
       * 7/29/19 - DAX - updating below to take lines in lieu of sentences
       */
      final boolean skipBlankLines = true;
      final List<RaptatTokenPhrase> tokenPhrases = this.tokenPhraseMaker
          .getTokenPhrases(documentLines, rawTokensAsTree, processedTokensAsTree, skipBlankLines);
      theDocument.setTokenPhrases(tokenPhrases);

      if (LOGGER.getLevel().equals(Level.DEBUG)) {
        LOGGER.debug("RAPTAT TOKEN PHRASES in " + theDocument.getTextSource());
        for (RaptatTokenPhrase phrase : tokenPhrases) {
          StringBuilder sb = new StringBuilder();
          sb.append("START:" + phrase.get_start_offset_as_int());
          sb.append("\t").append("STARTLINE:" + phrase.get_start_offset_relative_to_line());
          sb.append("\t").append("END:" + phrase.get_end_offset_as_int());
          sb.append("\t").append("ENDLINE:" + phrase.get_end_offset_relative_to_line());
          sb.append("\t").append(
              "STRING:" + RaptatToken.getUnprocessedStringFromTokens(phrase.getTokensAsList()));
          System.out.println(sb.toString());
        }
      }

      final List<DocumentColumn> documentColumns =
          VerticalColumnAlgorithm.invoke(tokenPhrases, false);
    ...
    /*
     * This is where to place checklist section marker for use in extracting semi-structured
     * elements within the text
     */
    this.medListSectionMarker.markDocumentSentences(theSentences, inputText);
    ...
    if (LOGGER.getLevel().equals(Level.DEBUG)) {
      ...
      int columnNumber = 0;
      for (DocumentColumn documentColumn : theDocument.getDocumentColumns()) {
        LOGGER.debug("\n\n\n----------------- COLUMN:" + columnNumber++ + " ---------------------");
        for (RaptatTokenPhrase phrase : documentColumn.columnPhrases) {
          System.out.println(
              "============== PHRASE STRING:  " + phrase.getPhraseText() + " ==============");
  ...
  public void setFacilitatorBlockerDictionary(final TokenContextAnalyzer dictionary) {
    this.facilitatorBlockerDictionary = dictionary;
  }

  /**
   * @param generateTokenPhrases the generateTokenPhrases to set
   */
  ...
  public synchronized void setTokenProcessingOptions(final OptionsManager optionsManager) {
   ...
   */
  public synchronized void setTokenProcessingParameters(final boolean useStems,
      final boolean usePOS, final boolean removeStopWords, final int maxElements,
  ...
  public synchronized void setTokenProcessingParameters(final TokenProcessingOptions theOptions) {
  ...
  public RaptatPair<List<String>, List<Integer>> splitByAlphanumeric(String stringToSplit,
    ...
    if (TextAnalyzer.nonAlphaTokenStart.find()) {
      final String startString = TextAnalyzer.nonAlphaTokenStart.group(1);
    ...
    while (TextAnalyzer.alphaNonAlphaAlternationMatcher.find()) {
      for (int i = 1; i < 3; i++) {
        final String curGroupString = TextAnalyzer.alphaNonAlphaAlternationMatcher.group(i);
  ...
  public void updateTrainingGroup(final AnnotationGroup inputGroup) {
    ...
    TextAnalyzer.LOGGER
        .debug("Regenerating processed tokens for document:" + raptatDocument.getTextSource());
    final List<RaptatToken> processedTokens =
    ...
    raptatDocument.setProcessedTokens(RaptatToken.generateIndexedTree(processedTokens));

    TextAnalyzer.LOGGER.debug("Adding processed tokens to document annotations");

    inputGroup.restoreSplitPhrases();
    final Hashtable<Integer, AnnotatedPhrase> splitPhrases =
   ...
   * @author Glenn Gobbel - Jul 13, 2012
   */
  public void updateTrainingGroups(final List<AnnotationGroup> documentGroups) {
   ...
   */
  @Deprecated
  private void assign_token_phrases_and_tokens_to_lines(final LinkedList<TextDocumentLine> lines,
      final List<RaptatTokenPhrase> phrases, final IndexedTree<IndexedObject<RaptatToken>> tokens) {

    /*
     * Load all the doc lines into a tree set ordered by end offset. This will permit searching for
     * keys based on the floor or ceiling of the phrase or token offset
     */
    ...
    for (final TextDocumentLine textDocumentLine : lines) {
      map_end_offset_to_line.put(textDocumentLine.endOffset, textDocumentLine);
    }
    ...
    for (final RaptatTokenPhrase raptatTokenPhrase : phrases) {
       ...
       */
      final int phrase_end_offset = raptatTokenPhrase.get_end_offset_as_int();
      /*
       * Get the next line, which in reality is the current line of this phrase
       */
      final TextDocumentLine current_line =
          map_end_offset_to_line.ceilingEntry(phrase_end_offset).getValue();
      /*
      ...
      if (map_line_to_tokens.containsKey(current_line) == false) {
        map_line_to_tokens.put(current_line, new ArrayList<RaptatTokenPhrase>());
      }
      final List<RaptatTokenPhrase> phrases_in_current_line = map_line_to_tokens.get(current_line);
    ...
    /**
     * The following just reviews the list of tokens and assigns the line to them; this may not be
     * needed, but it can be torn out easily
     */
    if (tokens != null) {
      for (final IndexedObject<RaptatToken> indexedObject : tokens.getAllNodes()) {
        /*
         * Get the end offset from the phrase
         */
        final RaptatToken indexed_token = indexedObject.getIndexedObject();
        /*
         * Get the current line of this phrase
         */
        final int end_offset = indexed_token.get_end_offset_as_int();
        final TextDocumentLine current_line =
            map_end_offset_to_line.ceilingEntry(end_offset).getValue();
        /*
         * set the reference to current line
         */
   ...
   * @param tokens
   */
  private void assignTokenPhrasesToLines(final List<TextDocumentLine> textDocumentLines2,
  ...
  private List<TextDocumentLine> getTextDocumentLines(final String inputText,
    ...
    final Matcher lineBreakMatcher = TextAnalyzer.simpleLineBreak.reset(inputText);
    int startOffset = 0;
    /**
    ...
    IndexedTreeIterator processedTokenIterator = processedTokensAsTree.iterator();
    ...
    while (lineBreakMatcher.find()) {
      ...
      final String lineString = inputText.substring(startOffset, endOffset);
      LOGGER.debug("Processing " + lineString);
      final TextDocumentLine line = new TextDocumentLine(startOffset, endOffset, lineString,
          processedTokenIterator, rawTokenIterator, priorLine, sourceDocument);
  ...
  /**
   * Excludes from the list of index values any phrases that fall within the bounds of an exclusion
   * phrase
   *
  ...
  public static List<RaptatTokenPhrase> filterIndexValueCandidatesByExcludedValues(
    ...
    /**
     * Check each possible candidate to see if it's contained in an exclusion
     */
    for (final RaptatTokenPhrase indexValueCandidate : indexValueCandidates) {
      ...
      for (final RaptatTokenPhrase exclusionPhrase : exclusionPhrases) {
        final boolean candidateStartIsGTEExclusionStart = indexValueCandidate
            .get_start_offset_as_int() >= exclusionPhrase.get_start_offset_as_int();
        final boolean candidateEndisLTEExclusionEnd =
            indexValueCandidate.get_end_offset_as_int() <= exclusionPhrase.get_end_offset_as_int();
        isWithinPhrase |= candidateStartIsGTEExclusionStart && candidateEndisLTEExclusionEnd;
        if (isWithinPhrase) {
          break;
        }
      }
   ...
   * @return
   */
  public static List<RaptatTokenPhrase> identifyIndexValueTokenPhrases(final String inputText) {
    final Pattern pattern = Pattern.compile(IndexValueConstants.IndexValueRegEx);
    ...
    while (matcher.find()) {
      ...
      final RaptatToken raptatToken =
          new RaptatToken(text, start.toString(), end.toString(), null, null);
      final Label label = Label.createUnassignedLabel();
      final RaptatTokenPhrase phrase = new RaptatTokenPhrase(raptatToken, label);
      phrase.trim();
      phrase.addPhraseLabel(RaptatTokenPhrase.Label.createIndexValueLabel());
      phrases.add(phrase);
    }
   ...
   * @return
   */
  public static List<RaptatTokenPhrase> identifyValuesToIgnore(final String inputText) {
    final Pattern pattern = Pattern.compile(IndexValueConstants.IndexValueIgnoreRegEx);
  ...
  public static void main(final String[] args) {
    ...
    SwingUtilities.invokeLater(new Runnable() {
        ...
        switch (runType) {
          ...
          case ANALYZE_DOCUMENT:
            ...
            final String pathToDocument =
                "C:\\Users\\gobbelgt\\Documents\\SubversionMatheny\\branches\\GlennWorkspace\\Projects\\RaptatBaseProject\\"
                    + "trunk\\src\\main\\resources\\examplefiles\\mtSampleForNumbers_190421.txt";
      ...
      private void analyzeDocument(final String pathToDocument, final String pathToDictionary) {
        final TextAnalyzer ta = new TextAnalyzer(pathToDictionary, null);
        final RaptatDocument doc = ta.processDocument(pathToDocument);

        for (final AnnotatedPhrase curPhrase : doc.getDictionaryAnnotations(false)) {
          System.out.println(curPhrase.toString());
        }

      }

      private void checkGetProcessedTokenStrings(final String startString) {
        final TextAnalyzer ta = new TextAnalyzer();
        final RaptatPair<List<String>, List<Integer>> results =
            ta.splitByAlphanumeric(startString, 0);
        ...
        final Iterator<Integer> integerIterator = results.right.iterator();
      ...
      private void checkLemmatization() {
        ...
        final String inputSentence =
            "The entities were overwhelming and ran behind the cats and dogs";
      ...
      private void checkSentences() {
        ...
        for (final String curSentence : sentences) {
          ...
          final List<AnnotatedPhrase> phrases = doc.getActiveSentences();
          final List<RaptatToken> sentenceTokens = phrases.get(0).getProcessedTokens();
