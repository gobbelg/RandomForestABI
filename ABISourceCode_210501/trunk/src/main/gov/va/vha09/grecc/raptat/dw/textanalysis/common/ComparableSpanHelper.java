...
public class ComparableSpanHelper {
  ...
  public static Comparator<RaptatTokenPhrase> overlapIsEqual = new Comparator<RaptatTokenPhrase>() {
    ...
    public int compare(RaptatTokenPhrase largerPhrase, RaptatTokenPhrase smallerPhrase) {
      ...
      int smallerPhrase_start = smallerPhrase.get_start_offset_as_int();
      int smallerPhrase_end = smallerPhrase.get_end_offset_as_int();
      int largerPhrase_start = largerPhrase.get_start_offset_as_int();
      /**
       * substract length of smaller from larger in order to find smaller start between larger
       * endpoints
       */
      int largerPhrase_end = largerPhrase.get_end_offset_as_int() - smallerPhrase_start;
  ...
  public boolean doesLhsPhraseCoverRhsPhrase(final RaptatTokenPhrase lhs,
    ...
    boolean rv = false;
    int rhs_end_offset_as_int = rhs.get_end_offset_as_int();
    int lhs_end_offset_as_int = lhs.get_end_offset_as_int();
    if (rhs_end_offset_as_int > lhs_end_offset_as_int) {
      int rhs_start_offset_as_int = rhs.get_start_offset_as_int();
      int lhs_start_offset_as_int = lhs.get_start_offset_as_int();
