...
import src.test.gov.va.vha09.grecc.raptat.dw.common.folders.SourceFolder;

/**
 * This is essentially the TDD_Convert_RaptatTokenPhrase_objects_into_eHOST_XML class that Dax
 * created copied from a test class to this class so that it can be easily run from the GirotraABI
 * class to show what RaptatTokenPhrase labels are being created.
 *
...
public class TokenPhraseWriter {
  ...
  public void writePhrases(final String textFolderPath, final String xmlOutputFolderPath,
      final String sentenceLogFolder) throws NotDirectoryException {

    // Create TextAnalyzer instance

    final TextAnalyzer textAnalyzer = new TextAnalyzer(null, null);

    final SourceFolder sourceFolder = new SourceFolder(textFolderPath, "txt");
    final XMLExporterRevised exporter =
        new XMLExporterRevised(AnnotationApp.EHOST, new File(xmlOutputFolderPath), true);
    ...
    while (sourceFolder.hasNext()) {
      ...
      final File fileToProcess = sourceFolder.next().baseFile();

      process(textAnalyzer, sentenceLogFolder, exporter, fileToProcess);
  ...
  /**
   * This is an alternative to another writePhrases method. This one allows providing a specific
   * TextAnalyzer. By doing so, the options and TokenPhaseMaker type of the TextAnalyzer instance
   ...
   */
  public void writePhrases(final String textFolderPath, final String xmlOutputFolderPath,
      final String sentenceLogFolder, final TextAnalyzer textAnalyzer)
      throws NotDirectoryException {
    ...
    while (sourceFolder.hasNext()) {
      ...
      final File fileToProcess = sourceFolder.next().baseFile();
      System.out.println("Processing file " + ++fileIndex + " of " + fileNumber + ":"
  ...
  /*
   * Convert the RaptatTokenPhrase objects to a list of AnnotatedPhrase objects by: a. Get the list
   * of Token objects within the RaptatTokenPhrase b. Use the constructor,
   * AnnotatedPhrase(List<RaptatToken>, RaptatDocument, String (annConcept), String (mentionID),
   * String (annotatorName), String (annotatorID), int (index), List<RaptatAttribute>,
   * List<ConceptRelation>) - List<RaptatToken> constituent tokens of phrase - int (index) is index
   * of phrase in list of, phrases found in RaptatDocument - List<RaptatAttribute>,
   * List<ConceptRelation> -> empty i. RaptatDocument is the document the RaptatTokenPhrases came
   * from ii. annConcept is either the _name or _value field of the RaptatTokenPhrase object
   * (_**e.g., ABI, ANATOMY, etc.**_) iii. mentionID is a unique identifier, which can be retrieved
   * via a call to UniqueIDGenerator.INSTANCE.getUnique() iv. annotatorName and annotatorID can both
   * be 'RaptatABITool' v. index is the number of the RaptatTokenPhrase object within the list of
   * RaptatTokePhrase objects when all are sorted in the list according to start offset vi. The list
   * of RaptatAttrbute objects and ConceptRelation objects is an empty list
   */
  private List<AnnotatedPhrase> convertRaptatTokenPhrasesToAnnotatedPhrases(
    ...
    for (int index = 0; index < tokenPhrases.size(); index++) {
      ...
      final List<RaptatToken> tokens = raptatTokenPhrase.getTokensAsList();
      final Set<Label> phraseLabels = raptatTokenPhrase.getPhraseLabels();
      for (final Label label : phraseLabels) {
        if (label.hasAssignment()) {
          final String mentionId = UniqueIDGenerator.INSTANCE.getUnique();
          final String annConcept = label.get_name() + "_" + label.get_value();
          final AnnotatedPhrase phrase = new AnnotatedPhrase(tokens, document, annConcept,
              mentionId, ANNOTATOR_NAME, ANNOTATOR_ID, index, new ArrayList<RaptatAttribute>(),
  ...
  private AnnotationGroup createAnnotatedGroups(final List<AnnotatedPhrase> annotatedPhrases,
      final RaptatDocument document) {
    final AnnotationGroup annoationGroup = new AnnotationGroup(document, annotatedPhrases);
  ...
  private void exportAnnotationGroups(final AnnotationGroup annotatedGroup,
    ...
    exporter.exportAnnotationGroup(inputGroup, correctOffsets, insertPhraseStrings,
   ...
   * @param outputFolder
   */
  private void logSentences(final RaptatDocument document, final String outputFolder,
    ...
    try (BufferedWriter writer = new BufferedWriter(
        new FileWriter(outputFolder + "\\" + fileName + ".sentences.txt", false))) {
      for (int index = 0; index < textDocumentLines.size(); index++) {
        final TextDocumentLine textDocumentLine = textDocumentLines.get(index);
        writer.write(MessageFormat.format("{0} {1}\r\n", index + 1, textDocumentLine.lineText));
      }
    } catch (final IOException e) {
      e.printStackTrace();
    }
  ...
  private void process(final TextAnalyzer textAnalyzer, final String sentenceLogFolder,
      final XMLExporterRevised exporter, final File fileToProcess) {
    // Get the list of RaptatTokenPhrase objects within the resulting
    // RaptatDocument.
    final RaptatDocument document = textAnalyzer.processDocument(fileToProcess.getAbsolutePath());
    ...
    final List<AnnotatedPhrase> annotatedPhrases =
        convertRaptatTokenPhrasesToAnnotatedPhrases(document);

    // convert to annotation groups
    final AnnotationGroup annotatedGroup = createAnnotatedGroups(annotatedPhrases, document);
